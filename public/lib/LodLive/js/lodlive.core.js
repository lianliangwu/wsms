...
(function($, lodLiveProfile) {
	...
	var methods = {
		...
		doStats : function(uri) {
			// wulianliang: change the status.html path
			if ($('#stats').length == 0) {
				$('body').append('<div id="stats"><iframe src="lib/LodLive/stats.html?' + uri + '" style="display:none"></iframe></div>');
			} else if ($('#stats').children('iframe').length == 10) {
				$('#stats').empty();
				$('#stats').append('<iframe src="lib/LodLive/stats.html?' + uri + '" style="display:none"></iframe>');
			} else {
				$('#stats').append('<iframe src="lib/LodLive/stats.html?' + uri + '" style="display:none"></iframe>');
			}
		},
		...
		format : function(destBox, values, uris, inverses) {
			...
			// wulianliang: edit the cycle text
			// ******************************** old version ****************************************** //
		...
		parseRawResource : function(destBox, resource, fromInverse) {
			...
			if (lodLiveProfile['default']) {
				...
				$.jsonp({
					url : url,
					beforeSend : function() {
						destBox.children('.box').html('<img style=\"margin-top:' + (destBox.children('.box').height() / 2 - 8) + 'px\" src="lib/LodLive/img/ajax-loader.gif"/>');
					},
					success : function(json) {
						json = json['results']['bindings'];
						var conta = 0;
					...
					success : function(json) {
						...
						if ($.jStorage.get('doInverse')) {
							...
							$.jsonp({
								url : SPARQLquery,
								beforeSend : function() {
									destBox.children('.box').html('<img style=\"margin-top:' + (destBox.children('.box').height() / 2 - 5) + 'px\" src="lib/LodLive/img/ajax-loader.gif"/>');
		...
		allClasses : function(SPARQLquery, destBox, destSelect, template) {
			...
			$.jsonp({
				url : SPARQLquery,
				beforeSend : function() {
					destBox.html('<img src="lib/LodLive/img/ajax-loader.gif"/>');
				},
				success : function(json) {
